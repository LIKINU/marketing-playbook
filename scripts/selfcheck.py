...
def main():
    ...
    # ⚠️ 2026-09-20 补：漏了单独的「禁用」二字 —— 而真实稿里最常见的写法是
    #   「禁用「治疗／药妆／医学级…」」（创意简报的 mandatories 字段就这么写）。
    #   只有「禁用词」三个字能认 → 那行里的词全被当违规（实测瞳话案的 6 个误报之一）。
    #   ⚠️ 2026-09-20 再补一批**否定/边界语境**词 —— 来源是真实稿（瞳话案）实测的两种漏网写法：
    #     · 「**不承诺**：任何"治疗/祛病"功效」——在声明不做，不是在宣称
    #     · 「话术越界提"治疗"」——在**风险表**里描述风险
    #   只收「明确的否定/元语言」词，**不收「不得／不能」**（那俩在普通行文里太常见，会过度豁免）。
    _BAN_LINE = ("禁用", "禁语", "禁止", "红线", "不能说", "替换说法",
                 "不承诺", "不提", "避免", "越界", "不做任何",
                 "禁", "放弃", "改用")   # ← 从「整块判定」挪到「按行判定」（防相邻表合并后误豁免）
    safe_idx = set()
    in_code = False
    ...
    while _i < len(lines):
        if lines[_i].strip().startswith("|"):
            ...
            while _j < len(lines) and lines[_j].strip().startswith("|"):
                _j += 1
            # ⚠️ 2026-09-20 修一处**真漏洞**（实测：单行 `|` 行整行逃过红线扫描）：
            #   原来**无条件**把每个 `|` 块的第一行当「表头（字段名）」排除 ——
            #   可是**只有一个 `|` 行的块根本没有表头**，那一行就是正文。
            #   于是 `| 主打卖点 | 能治疗近视 |` 这种单行写法（手写、或表被压成一行）
            #   **整行不在扫描范围内** → 真违规可以藏在这里。
            #   → 只有**真正的表（≥2 行）**才把首行当表头排除；单行块照常扫。
            if _j - _i >= 2:
        ...
        if lines[_i].strip().startswith("|"):
            ...
            for _k in range(_i - 1, -1, -1):
                if lines[_k].strip():
                    _prev = lines[_k]
                    break
            # ⚠️ 2026-09-20 补两类**表头没写「禁用」、但内容就是在讲禁用**的表（实测瞳话案 6 个误报的根因）：
            #   ① **合规/放弃类**：表头写「硬约束｜因此主动放弃（动作名）｜…」，
            #      数据行写「禁『治疗／药妆／抗敏』／放弃『药妆』」→ **该块文本里有「禁/放弃/改用/替换说法」
            #      这类否定或替代语境**，整块豁免（一张在说「哪些词别用」的表，不该被自己的判据举报）。
            #   ② **资质/证照类**：表头写「资质名称｜发证机关｜要点」，数据行里有
            #      「染发／烫发／祛斑美白／防晒等特殊类」→ 那是**法规品类名**，不是功效宣称 → 表头含资质类词即豁免。
            #   ⚠️ 2026-09-20 修正：原来还加了一条「整块文本里出现 禁/放弃/改用 就豁免」——
            #     **那条改过头了**：这类稿里相邻表格常无空行、会被并成一个大块，
            #     于是一个大块里某处有「禁」字，**整块（含真违规）全被豁免**（实测漏拦）。
            #     → 挪到**按行**判：只有「含禁用词的那一行自己」有否定/替代语境才豁免（见 _BAN_LINE）。
            #   ③ **风险/合规类**：表头「风险场景｜发生原因｜预警信号｜兜底预案｜预防动作」——
            #      里面写「话术越界提『治疗』」是在**描述风险**，不是在宣称（同一次实测提出来的）。
            _QUALIFY = ("资质", "证照", "发证机关", "注册证", "证书编号", "备案",
                        "风险场景", "预警信号", "兜底预案", "预防动作", "违规后果")
            if (any(m in _hdr or m in _prev for m in _BAN_MARK)
                    or any(q in _hdr for q in _QUALIFY)):
                for _k in range(_i, _j):
                    safe_idx.add(_k)
            _i = _j
            continue
    ...
    # ⚠️ 2026-09-20 校准：**词表太窄会误伤** —— 实测一条合格的写法
    #   「把 VI **规范**…交给招牌**制作商**…验收看色差 ≤5%」被判「要素不齐」，
    #   因为词表里没有「规范」「制作商」这两种常见写法。→ 扩词 + **判定放宽到「命中 ≥2 类」**
    #   （启发式判据天然误伤，标准要留余量；真正的硬门槛交给**字数**）。
    _HOW4 = (r"用|先|把|按|做|拆|列|排|测|跑|拍|写|画|建|约|谈|发|投|算|对|核",              # ① 方法
             r"工具|模板|表格|表单|清单|话术|脚本|brief|SOP|规范|手册|物料|素材|"
             r"系统|软件|App|小程序|Excel|看板|表单",                                          # ② 工具／物料
             r"谁|岗|人|团队|部门|老板|店长|客户|顾问|督导|"
             r"由.{0,6}(做|负责|执行|承担|落)|制作商|供应商|施工|外包|执行方|对接人",            # ③ 谁执行
    ...
    for _pi, _b in enumerate(p_blocks, 1):
        ...
        _weak4 = [j for j, _h in enumerate(_how, 1) if _zh(_h) >= _MIN_HOW
                  and sum(bool(re.search(_p, _h)) for _p in _HOW4) < 2]   # 命中 <2 类才算「要素不齐」
        if _ok_n < _ns:
    ...
    if _thin4 and not quiet:
        print(f"  {NG} 有 {len(_thin4)} 处「怎么做」**要素不齐**（四件事只命中 ≤1 类）："
              f"{'、'.join(_thin4[:4])}{'…' if len(_thin4) > 4 else ''}")
    if _thin4:
        _hard_if_full(
            f"这些步骤的「怎么做」**只说了一半**：{'、'.join(_thin4[:6])} —— "
            "「怎么做」至少要覆盖四件事里的两类：① 用什么方法　② 用什么工具／模板／物料　"
            "③ 谁在什么条件下执行　④ 做到什么算完成（带数字或判据）。命中不到两类＝还是空泛。")
    if _unfilled:
        if not quiet:
    ...
    #     `grep 直播|排品` 全仓零命中）。
    #    但**只加章节、不加判据 ＝ 又回到「骨架给了位、脚本查不到」**（已知坑 5）——
    ...
    # 33) 逐节深度下限（**2026-09-20 新增 · 从真实稿反查出来的**）
    #
    #    来源：瞳话大赛 v2（骨架填齐、98 个标题，但 **24 个一级节里 17 个只有 100–600 实字**：
    #    产品货盘 150／内容矩阵 115／控价 112／工具底座 133… 而「策略」一节 8,577 字吃掉全文 34%）。
    #    用户原话：「章节里的内容都很分散，一句两句的」「只讲了有什么可以做什么，不够详细」。
    #
    #    ⚠️ 这是**新增 policy**，不是兑现旧承诺 —— 已核对：范式库（434 节指引）与
    #      `09-完整策划标准与评分表` 里**都没有**「每节 ≥N 字」这类表述。
    #      所以门槛取**保守**：只抓「明显过薄」，并给附录／参考类豁免；速览档走 `_hard_if_full`。
    #      符合本库规矩：新硬关先问「这条对最轻的那档也成立吗」。
    #
    #    为什么按**节**按**实字**：章节是填稿人真正会跳过的单位（跳一节＝少一块动作）；
    #    实字（汉字数）比总字符稳 —— 表格分隔线、编号、标点都不算，不会因为「表多」而虚胖。
    _MIN_SEC = 300
    _MIN_ROWS = 6        # 或者：该节有 ≥6 行表格数据（见下「表格也算内容」）
    _EXEMPT_SEC = ("附录", "附 A", "附 B", "参考", "索引", "目录", "封面", "变更记录", "速查",
                   "摘要", "TL;DR", "结论卡片", "卡片")   # 摘要类**功能就是短**，不该算薄
    _thin33 = []
    for _m33 in re.finditer(r"(?ms)^##\s+([^\n]*)\n(.*?)(?=^##\s|^#\s|\Z)", body):
        _nm33 = _m33.group(1).strip()
        if any(e in _nm33 for e in _EXEMPT_SEC):
            continue
        _b33 = _m33.group(2)
        _n33 = _zh(_b33)
        # ⚠️ **表格也算内容**：实测（2026-09-20 用真标杆稿当反向夹具）发现只看汉字会**误伤表格式章节** ——
        #   「三 · 节奏排期」106 字、「八 · KPI 与追踪机制」230 字、「六 · 触达与渠道」202 字
        #   都是**内容在表里**（周次／指标／渠道全是数字与短词，汉字天然少）。
        #   → 判据改成「**汉字 ≥ 300 或 表格数据行 ≥ 6**」，二者其一即算够厚。
        _rows33 = len([r for r in _b33.split("\n")
                       if r.strip().startswith("|") and "---" not in r])
        _rows33 = max(0, _rows33 - 1)          # 减掉表头行
        if _n33 < _MIN_SEC and _rows33 < _MIN_ROWS:
            _thin33.append((_nm33[:22], _n33, _rows33))
    if not quiet:
        _show33 = "、".join(f"{n}（{c} 字／{r} 表行）" for n, c, r in _thin33[:6])
        print(f"  {OK if len(_thin33) < 3 else NG} 逐节深度：{len(_thin33)} 节过薄"
              f"（< {_MIN_SEC} 实字 且 < {_MIN_ROWS} 表行）"
              + (f"｜{_show33}{'…' if len(_thin33) > 6 else ''}" if _thin33 else ""))
    if len(_thin33) >= 3:
        _hard_if_full(
            f"{len(_thin33)} 个一级章节**过薄**（既不足 {_MIN_SEC} 实字、也没有 {_MIN_ROWS} 行表格）："
            + "；".join(f"{n} {c} 字／{r} 表行" for n, c, r in _thin33[:6])
            + ("…" if len(_thin33) > 6 else "")
            + f"）—— **章节过薄 = 只写了「有什么可做」，没写「怎么做、做多少、谁做、怎么验收」**。"
              f"逐节补齐到 ≥{_MIN_SEC} 实字（要含具体动作与数字），或把该节并进相邻章节，不要留成标题＋一两句。")
    elif _thin33:
        warnings.append(f"有 {len(_thin33)} 个一级章节偏薄（< {_MIN_SEC} 实字且无表）："
                        + "、".join(f"{n} {c} 字" for n, c, _r in _thin33))

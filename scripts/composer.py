...
def play_block(i, p, kmap, ind=""):
    ...
    if steps:
        ...
        for k, (act, out) in enumerate(steps, 1):
            ...
            # ⚠️ 2026-09-20：**不能只放一个【填】就完事** —— 用户实测反馈「还是太短了，
            #   不够具体，内容太少了，要在代码级改」。空占位符＝没有规格，填的人只会写一句。
            #   → 把**填写规格**写进骨架（这是代码级的约束，不靠人记）：
            #     3–5 句 · 四件事（方法 → 工具/模板 → 谁在什么条件下做 → 做到什么算完成·要带数字或判据）。
            s_lines.append(
                f"     - **怎么做**：{FILL}（**写 3–5 句、≥40 字**，四件事缺一不可："
                f"① 用什么方法　② 用什么工具／模板／物料　③ 谁在什么条件下执行　"
                f"④ 做到什么算完成——要带**数字或判据**。少于 40 字＝没写）")

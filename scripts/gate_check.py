...
用途：AI 问完门禁后、动笔写方案前，跑一次这个脚本。
      门禁 13 项有任何一项没填、或《任务规则表》没经用户确认 → 直接报错，不准往下走。
      ⚠️ **第 13 项就是「交付规格」（结构／量级／字数三子字段），载体是 JSON 的 `delivery`** ——
         不再单列「第 14 项」（2026-09-22 合并：文档一直写 13 项而脚本实查 14 项，
         且同一信息要在 `gate.目标字数` 与 `delivery` 填两处）。旧字段仍兼容，不报错。
...
GATE_ITEMS = [
    ...
    # 第 13 项（2026-09-22 口径统一）：＝**交付规格**，载体是 `delivery` 的三子字段
    #   （结构／量级／字数）。旧的 `gate.目标字数`／`页数` 仍作别名兼容。
    ("交付规格（结构／量级／字数三子字段；载体＝delivery）",
     ["交付规格", "交付", "delivery", "结构", "量级", "目标字数", "字数", "页数", "字数限制", "篇幅"]),
]
...
def main():
    ...
    # ── 第 13 项「交付规格」的载体是 `delivery`（2026-09-22 合并原「第 14 项」）──
    #   为什么这样合：原来 `delivery` 是**单列的第 14 项**，于是
    #     ① 文档通篇写「13 项」，脚本却多查一项（口径不符，读者以为已问全）；
    #     ② 同一信息要在 `gate.目标字数` 与 `delivery.量级` **填两处**（范例 JSON 真的两处都有）。
    #   → 现在：`delivery` 就是第 13 项的载体；旧字段仍走别名，不报错。
    ...
    print(f"{OK} 门禁通过：13 项齐全（第 13 项＝交付规格，载体 delivery）+ 规则表已确认。")
    ...
    # ── 交付规格的取值说明（原「第 14 项」已并入第 13 项，2026-09-22）──
    #   第 13 项缺失时，上面的 `missing` 里会列出它 —— 提示语在这里补齐（因为它有特定的问法）。
    if any("交付规格" in m for m in missing):
        print()
        print("→ 第 13 项「交付规格」的问法（**必须给选项让用户选，不许开放式地问「你要多长」**）：")
        print("   例：\"delivery\": {\"结构\": \"精炼版\", \"量级\": \"5 页 Word\", \"字数硬要求\": \"无\"}")
        print("   三子字段＝结构（精炼版／标准版／完整版）＋量级（页数或字数）＋字数硬要求（无／上限）。")
    ...
    print(f"{OK} 门禁通过：13 项齐全（含第 13 项交付规格）＋ 规则表已确认 → 可以进入下一步（事实底稿）。")
    sys.exit(0)
